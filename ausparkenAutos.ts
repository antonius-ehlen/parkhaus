// Autos einparken
import { Parkhaus, Platz } from "./interfaces"

function ausparkenAutos(parkhaus: Parkhaus, anzahl: number) {
    if(anzahl > parkhaus.anzEtagen * parkhaus.plaetzeProEtage) {
        anzahl = parkhaus.anzEtagen * parkhaus.plaetzeProEtage
    }
    console.log(' ausparken von', anzahl, 'Autos!')
    // anzahl ist anzahl aufahrender autos
    let belegtePlaetze: Platz[] = []
    let gefunden: Platz[]
